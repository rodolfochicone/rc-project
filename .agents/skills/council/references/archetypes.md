...
Each archetype is a canonical reusable agent resolved by name from `.rc/agents/<name>/` or `~/.rc/agents/<name>/`. `rc setup` provisions the standard council roster globally under `~/.rc/agents/`. Dispatch these archetypes through the host-owned `run_agent` tool. Do not summarize or reinvent their priorities — dispatch the actual reusable agent so its authentic voice is preserved.
