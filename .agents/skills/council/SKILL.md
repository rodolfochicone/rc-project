...
2. Treat each selected advisor as a canonical reusable agent id. The standard roster is provisioned by `rc setup` under `~/.rc/agents/<name>/` and can be overridden per workspace under `.rc/agents/<name>/`.
...
- **`run_agent` is unavailable or the council archetypes are missing**: stop and tell the user to run `rc setup` so the global council agents are provisioned under `~/.rc/agents`.
