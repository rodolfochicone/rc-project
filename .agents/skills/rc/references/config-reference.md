...
Complete reference for `.rc/config.toml` workspace configuration.

## File Location

Place the configuration file at `.rc/config.toml` in the workspace root. CLI flags always override config values.
...
| `model` | string | Model override. Per-IDE defaults: codex/droid=gpt-5.5, claude=opus, copilot=claude-sonnet-5, cursor-agent=composer-1, opencode/pi=anthropic/claude-opus-4-6, gemini=gemini-2.5-pro |
...
Options specific to `rc tasks run`.

| Field | Type | Description |
| --- | --- | --- |
| `include_completed` | bool | Include tasks already marked as completed |
| `task_runtime_rules` | `array<table>` | Type-scoped runtime overrides applied after `[defaults]` for `rc tasks run` |

#### `[[tasks.run.task_runtime_rules]]`

Per-task runtime rules let `rc tasks run` change the runtime for tasks that match a given task `type`. This v1 config surface is intentionally bulk-oriented: config supports `type` selectors only, while one-off task `id` overrides are available from the CLI and TUI for the current run.
...
Options specific to `rc reviews fix`.
...
Options specific to `rc reviews fetch`.
...
Options specific to `rc reviews watch`. Watch-specific loop values come from this section; child review
...
```bash
rc reviews watch --provider coderabbit --pr 123
rc reviews watch my-workflow --provider coderabbit --pr 123 --max-rounds 4
rc reviews watch my-workflow --provider coderabbit --pr 123 --auto-push --push-remote origin --push-branch feature/reviews
```
...
Options specific to `rc exec`. Inherits all `[defaults]` fields plus:

| Field | Type | Description |
| --- | --- | --- |
| `verbose` | bool | Emit operational runtime logs to stderr |
| `tui` | bool | Open the interactive TUI |
| `persist` | bool | Save artifacts under `~/.rc/runs/<run-id>/` |

### `[sound]`

Optional audio notifications that play when a run reaches a terminal state. Applies to both `rc tasks run` and `rc exec`. Disabled by default — setting any field without `enabled = true` is a no-op.
