...
2. **Run setup.** Execute `rc setup` to install core skills into target agents plus setup assets from enabled extensions. For a quick start: `rc setup --all`.
3. **Install optional ideation extension when needed.** To use `/rc-idea-factory`, run `rc ext install --yes rc/rc --remote github --ref <tag> --subdir extensions/rc-idea-factory`, then `rc ext enable rc-idea-factory`, then `rc setup` again.
...
6. Validation runs via `rc tasks validate`.
...
**Command:** `rc tasks run <slug> --ide <runtime>`
...
**Command:** `rc reviews fetch <slug> --provider coderabbit --pr <N>`
...
**Command:** `rc reviews fix <slug> --ide <runtime>`
...
**Command:** `rc exec [prompt]`

Execute a single prompt outside the pipeline workflow.

- **Reusable agents:** `rc exec --agent council "Evaluate this design"` invokes a named agent.
