...
description: Expands a raw idea into a structured, research-backed spec in .rc/tasks/<slug>/_idea.md through interactive brainstorming, web research, business analysis, and multi-advisor debate. Use when the user has a feature idea and wants to explore and structure it before creating a PRD. Do not use for PRD creation, technical specifications, task breakdown, or code implementation.
...
1. **Determine project & directory** — derive slug, create `.rc/tasks/<slug>/` and `adrs/`
...
9. **Save the file** — write to `.rc/tasks/<slug>/_idea.md`
...
5. Debate trade-offs through multi-advisor council.
   ...
   - If `run_agent` is unavailable or the council archetypes cannot be resolved, stop with an actionable error that tells the user to run `rc setup`.
   ...
   - After the debate, create an ADR for the scope decision:
     ...
     - Write the ADR to `.rc/tasks/<slug>/adrs/adr-NNN.md` (zero-padded 3-digit number).
...
9. Save the idea file.
   ...
   - Ask the user to confirm the filename using the interactive question tool:
     - "Save as `.rc/tasks/<slug>/_idea.md`? (A) Yes / (B) Different name"
   - Write the file to `.rc/tasks/<slug>/_idea.md`.
   - Confirm the file path to the user.
   - Remind the user that the next step is to create a PRD using `rc-create-prd` from this idea.
...
- **Pipeline awareness** — The idea feeds into `rc-create-prd`; focus on WHAT and WHY, not HOW
