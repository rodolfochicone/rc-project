...
- Workspaces: `packages/ui` (`@escaletech/ui`), `web` (`rc-web`), `sdk/*`.
...
### `packages/ui` (`@escaletech/ui` — shared component lib; reuse, do not fork)
...
- Reuse `@escaletech/ui` (base-ui) components; do not introduce shadcn/radix or rewrite `web/`.
