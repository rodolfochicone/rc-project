...
- **ALWAYS USE** the `rc-final-verify` skill before claiming any task is done
- **Skipping any verification check will result in IMMEDIATE TASK REJECTION**

## Project Overview

rc is a Go module and CLI that drives the full lifecycle of AI-assisted development. It covers product ideation (PRD creation), technical specification, task breakdown with codebase-informed enrichment, and automated execution of each task via AI coding agents (Claude Code, Codex, Droid, Cursor). It also handles PR review remediation workflows.

The same skills and slash commands are also distributed as a **Claude Code plugin** (manifests under `.claude-plugin/`) — an additive, Claude-only install channel that auto-updates via the plugin marketplace. It does not replace `rc setup`, which remains the channel for all other agents. See the README's "Install as a Claude Code plugin" section.

## Package Layout

| Path                          | Responsibility                                                              |
| ----------------------------- | --------------------------------------------------------------------------- |
| `cmd/rc`                      | Standalone CLI entry point                                                  |
| `cmd/rc-plugin-sync`          | Release-time tool that pins the Claude Code plugin manifests to the version |
| `rc.go`                       | Public Go API — `NewCommand()` embeds the CLI as a Cobra subcommand         |
...
| `pkg/rc/events`               | Public event envelope, kind constants, and documented payload API           |
| `pkg/rc/runs`                 | Public run reader library for list/open/replay/tail/watch flows             |
...
| `.rc/tasks`                   | Default workflow artifact root (PRD, TechSpec, ADR, reviews)                |
...
- **TOOL HIERARCHY**: Use tools in this order:
  1. **Serena MCP symbolic tools** (when available) — `get_symbols_overview`, `find_symbol`, and `find_referencing_symbols` to understand code, and `replace_symbol_body` / `insert_after_symbol` / `insert_before_symbol` for precise edits. LSP-accurate and token-efficient; prefer over whole-file reads and line-based edits for symbol-level work.
  2. **Grep** / **Glob** — for plain-text (non-symbol) search and as the fallback when Serena is unavailable
  3. **`find-docs` skill** — for external Go libraries and framework documentation
  4. **Web search tools** — for web research, latest news, code examples
...
| Task completion         | `rc-final-verify`                                                               |                                                                                      |
...
1. Activate `rc-final-verify` skill
2. Run `make verify`
...
3. **Forget `rc-final-verify`** before marking tasks done
