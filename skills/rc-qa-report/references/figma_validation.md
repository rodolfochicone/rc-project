...
Guide for validating UI implementation against Figma designs using Figma MCP. When executing validations in a live browser, use the `agent-browser` CLI from the `rc-qa-execution` companion skill.
...
- `agent-browser` CLI installed (from `rc-qa-execution` companion skill)
- Dev server running
...
Use the standard viewports shared with `rc-qa-execution`: 375px (mobile), 768px (tablet), 1280px (desktop).
...
Use the unified bug report format from `assets/issue-template.md` (shared with `rc-qa-execution`). Store bug files in `<qa-output-path>/issues/`.
