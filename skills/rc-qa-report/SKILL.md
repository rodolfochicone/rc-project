---
name: rc-qa-report
description: Generate comprehensive test plans, test cases, regression test suites, automation annotations, and bug reports for QA engineers. Includes Figma MCP integration for design validation. Use when planning QA before execution, documenting test strategies, marking which flows require E2E follow-up, or creating structured bug reports. Do not use for executing tests against a live repository or running verification gates — use rc-qa-execution for that.
trigger: explicit
...
Plan and document QA deliverables — test plans, test cases, regression suites, Figma validations, and bug reports — in a structured format compatible with `rc-qa-execution` execution.

## Required Inputs

- **qa-output-path** (optional): Directory where all QA artifacts are stored. When provided, create the directory if it does not exist. When omitted, use the current working directory. This path must match the same argument passed to `rc-qa-execution` when both skills are used together.

## Shared Output Structure

All artifacts follow this directory layout, shared with `rc-qa-execution`:
...
└── verification-report.md  # Generated by rc-qa-execution
```
...
4. Mark automation candidates explicitly:
   ...
   - Tag bug-driven public regressions as `Automation Status: Missing` until `rc-qa-execution` confirms the spec was added or updated.
...
6. Use `agent-browser` (from the `rc-qa-execution` companion skill) when browser-based verification is needed. The core loop is: open → snapshot → interact → re-snapshot → verify.

**Step 7: Create Bug Reports**

1. Use the unified bug report format from `assets/issue-template.md`, shared with `rc-qa-execution`.
...
5. Cross-reference against `../rc-qa-execution/references/checklist.md` for coverage gaps when planning for later execution.
...
## Companion Skill: rc-qa-execution

The `rc-qa-report` and `rc-qa-execution` skills share a common output directory and artifact format. The intended workflow:

1. **Plan first** with `rc-qa-report`: generate test plans, test cases, and regression suites.
2. **Execute** with `rc-qa-execution`: run verification gates, exercise flows end-to-end, discover bugs, and add or update E2E coverage when the repository already supports it.
3. **Document** with `rc-qa-report`: create structured bug reports for issues found during execution.

When `rc-qa-execution` runs after `rc-qa-report`, it reads test cases from `<qa-output-path>/qa/test-cases/` to inform its execution matrix, automation priorities, and reporting fields, then writes bugs to `<qa-output-path>/qa/issues/` using the same unified template.
