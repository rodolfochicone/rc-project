---
name: rc-refactoring-analysis
description: >
  Analyzes codebases to identify refactoring opportunities based on Martin Fowler's catalog
  of code smells and refactoring techniques, AND performs a deep architectural audit for
  dead code, duplicated functionality, architectural anti-patterns, and type confusion.
  Detects duplicated code, high coupling, complex conditionals, primitive obsession, long
  functions, dead code, and other structural issues. Produces a structured report with
  prioritized findings saved to docs/_refacs/. Use when auditing code quality or codebase
  health, finding dead code, assessing architecture, or preparing a refactoring sprint.
  Don't use for style/formatting issues, performance optimization, or security audits.
model: opus
effort: high
---
...
## Architectural Audit (dead code, anti-patterns, type confusion)

When the request is a broader **architectural/codebase-health audit** — not just Fowler-style
refactoring — follow the structural-audit methodology in
`references/architectural-audit.md`. It complements the smell detection above with:
codebase-structure mapping and file inventory, entry-point/reachability analysis for **dead
code**, detection of **duplicated functionality** across modules, **architectural
anti-patterns**, and **type confusion**. Fold its findings into the same prioritized report
(Step 8) so a single audit covers both refactoring opportunities and architectural issues.
