...
description: Runs an optional cross-model peer review of an implemented change by delegating to a fresh rc-oracle subagent (Opus) and packages findings for user-directed remediation. Use after any implementation pass (feature, bug fix, refactor) when the user explicitly asks for an independent Opus review of the diff before commit or PR. Do not use for automatic remediation, batched provider review fetching (use rc-fix-reviews), manual self-review (use rc-review-round), or auto-looped review cycles.
trigger: explicit
...
A fresh `rc-oracle` subagent (Opus, read-only) pressure-tests an implementation diff. This skill runs that pressure-test only when the user explicitly asks for a review round after an implementation pass. It is decoupled from any PRD/task tracking system — the scope is the diff itself plus any optional context files the user names. The skill never auto-runs, auto-incorporates findings, auto-commits, or auto-loops additional rounds.
...
1. Read `references/impl-review-prompt.md` for the canonical Opus prompt template.
...
**Step 3: Execute the Cross-Model Review**

1. Delegate the assembled prompt to a **fresh `rc-oracle` subagent** (Opus, read-only) via the Task/Agent tool, instructing it to return the four JSON sections (`blockers`, `risks`, `nits`, `verdict`). A fresh subagent keeps the review independent of the implementation context.
2. Write the subagent's structured output to `<out>/impl-review-result-roundN.json`.
3. If the subagent errors or returns empty, fail loudly — do not retry silently or declare `SHIP` on empty output (see Error Handling).
...
- The `rc-oracle` review is the only external review pass this skill runs. Invoke it once per round.
...
- **Reviewer agent unavailable:** if the `rc-oracle` agent is not registered on the host, fall back to a general subagent pinned to a strong model (Opus). If the host has no subagent capability at all, tell the user this skill requires one rather than degrading the review to the same model that wrote the code.
