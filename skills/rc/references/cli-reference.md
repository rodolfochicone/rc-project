...
| --- | --- | --- | --- |
| `--workspace` | string | | Directory to resolve the `.rc` workspace from (default: current directory) |

**Workspace discovery.** rc resolves the active `.rc` directory by walking **up** from the current directory (or from `--workspace`) to the nearest `.rc`. If none is found above, it searches **below** for `.rc` directories (skipping `node_modules`, `.git`, `vendor`, and `_archived`): exactly one is used automatically, and several abort with an error listing the candidates — `cd` into the target subproject or pass `--workspace <dir>`. In monorepos with more than one `.rc`, run from the relevant subproject or use `--workspace` to pick one.
