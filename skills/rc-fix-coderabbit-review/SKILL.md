---
name: rc-fix-coderabbit-review
description: End-to-end remediation workflow for PR review feedback by PR number. Use when Codex must export CodeRabbit issues for a PR, fix every issue completely, commit all fixes in a single commit, and resolve GitHub review threads afterward. Do not use for provider-agnostic remediation from existing review round files (use rc-fix-reviews), PRD task execution, or reviews not addressed by PR number.
model: sonnet
effort: high
---
