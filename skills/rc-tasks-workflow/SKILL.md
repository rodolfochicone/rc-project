...
argument-hint: "[slug-or-jira-key]"
model: sonnet
effort: medium
---
...
2. If it is a **Jira key** (the rc-jira gancho), grep `jira_key:` across `.rc/tasks/**/task_*.md`; the matching files share one directory — that directory name is the slug. No local match → stop and say so (local task files are the source of truth; never reconstruct tasks from Jira text).
...
When this skill was reached from the **rc-jira *Execute a card*** flow, hand the per-task evidence back to rc-jira so it can post it to Jira (comment per sub-task + parent summary). This skill itself does **not** write to Jira.
...
- **Local task files are the source of truth.** Resolve the slug from local files; never reconstruct tasks from Jira text.
