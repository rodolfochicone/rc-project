...
- `type`: Allowed work type slug. Use `[tasks].types` from `.rc/config.toml` when configured; otherwise use the built-in defaults `frontend`, `backend`, `docs`, `test`, `infra`, `refactor`, `chore`, `bugfix`.
