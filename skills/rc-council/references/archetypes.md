...
Each archetype is a canonical reusable agent that ships with the rc plugin (the plugin's `agents/` directory) and is resolved by the host by id; a workspace may override an archetype by defining an agent with the same id in its own agents directory. Dispatch these archetypes through the host's agent-dispatch tool. Do not summarize or reinvent their priorities — dispatch the actual reusable agent so its authentic voice is preserved.
